...
func (c *Client) GetCardInfo(id string) (*CardInfo, error) {
	...
	if result.ResultCode > 0 {
		return nil, fmt.Errorf("%s (code: %d)", result.ErrorMessage, result.ResultCode)
	}

	return result, nil
}

func (c *Client) QueryCards(key string, iccids []string) (*CardInfos, error) {
	param := url.Values{}
	param.Add(key, strings.Join(iccids, ","))
	param.Add("method", "fc.function.cards.info")

	result := new(CardInfos)
	err := c.post(param, &result)
	if err != nil {
		return nil, err
	}
